...
| Database | **PostgreSQL** | Strapi's recommended production DB. Strapi Cloud manages it for you. **Strapi v5 is SQL-only — PostgreSQL / MySQL / MariaDB / SQLite. MongoDB and other document/NoSQL databases are NOT supported.** SQLite is fine *only* for local dev — never spec it for production. |
...
| Auth | **Stock Users & Permissions** (Better Auth plugin as opt-in) | U&P ships with Strapi, is production-ready, and matches the default Cloud-deploy path — most interviews should land here. The **`@strapi-community/plugin-better-auth`** plugin brings Better Auth (https://better-auth.com) — modern session/social/2FA — but is ⚠️ **beta, not for production per its maintainers** (needs Strapi ≥ 5.45, removes U&P). Offer it as the opt-in when the user wants those features and accepts POC risk. See `auth-better-auth.md`. Plugin repo: https://github.com/strapi-community/plugin-better-auth |
...
- **User insists on MongoDB or another non-SQL / document database** → Strapi v5 is **SQL-only** and cannot use Mongo. This is a hard misfit — surface it in **stage 1**, not stage 4: either switch the DB to PostgreSQL/MySQL, or, if a document model is a genuine hard requirement, Strapi is the wrong backend and they should use a more general product-design skill. Don't carry "MongoDB" forward as if it were a stage-4 toggle.
...
- **User wants social login / 2FA / passkeys and accepts POC risk** → offer the beta **better-auth plugin** (needs Strapi ≥ 5.45, removes U&P). Otherwise stay on the stock **Users & Permissions** default. Document the chosen auth in stage 5.
- **User is committed to NextAuth/Clerk/Auth0** → use that instead. Document the chosen auth in stage 5.
