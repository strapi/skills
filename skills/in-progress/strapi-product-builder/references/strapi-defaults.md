...
| Area | Default | Why |
|---|---|---|
| Backend / CMS | **Strapi v5** | Headless CMS with admin UI, REST + GraphQL, content types map cleanly from stage-3 entities. Companion skills (`strapi-configuration`, `add-page`, `dockerize-strapi`, `strapi-custom-field`) all assume Strapi v5. |
...
| API style | **Strapi REST** with route middlewares for default population | Matches Strapi's defaults; companion skills expect REST. Add the GraphQL plugin only if the frontend has a real reason. |
...
| Styling | **Tailwind** | Default in companion skills. |
...
- **User has compliance/data-residency needs Strapi Cloud doesn't meet** → keep Strapi v5, switch to self-hosted (Docker on the user's own infra). Invoke `dockerize-strapi` in stage 6 instead of the Strapi Cloud deploy step.
